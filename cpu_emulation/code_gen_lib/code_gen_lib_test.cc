...
void TestTrampoline(void* arg, ThreadState* state) {
  g_called = true;
  ASSERT_EQ(&g_arg, arg);
...
TEST(CodeGenLib, GenTrampolineAdaptor) {
  ...
  GenTrampolineAdaptor(
      &machine_code, ToGuestAddr(&g_insn), AsHostCode(TestTrampoline), &g_arg, "TestTrampoline");
...
void GenMoveResidenceToReg(MachineCode* machine_code) {
  ...
  as.Movl(as.edx, {.base = as.ebp, .disp = offsetof(ThreadState, residence)});
  as.Movl({.base = as.ebp, .disp = offsetof(ThreadState, cpu.r[0])}, as.edx);
  as.Jmp(kEntryExitGeneratedCode);
}

uint32_t GetResidenceReg(const ThreadState& state) {
  return state.cpu.r[0];
}
...
void TestRunner2(GuestAddr pc, GuestArgumentBuffer* buf) {
  g_called = true;
...
TEST(CodeGenLib, GenWrapGuestFunction) {
  ...
  GenWrapGuestFunction(
      &machine_code, ToGuestAddr(&g_insn), "vi", AsHostCode(TestRunner2), "TestRunner2");
...
void Run10Int8(GuestAddr pc, GuestArgumentBuffer* buf) {
  ...
  ASSERT_EQ(buf->argc, 10);
  ASSERT_EQ(buf->resc, 1);
  // For 8-bit parameters, only least-significant bits are defined!
  ASSERT_EQ(static_cast<uint8_t>(buf->argv[0]), 0U);
  ASSERT_EQ(static_cast<uint8_t>(buf->argv[1]), 0xffU);
  ASSERT_EQ(static_cast<uint8_t>(buf->argv[2]), 2U);
  ASSERT_EQ(static_cast<uint8_t>(buf->argv[3]), 3U);
  ASSERT_EQ(static_cast<uint8_t>(buf->argv[4]), 4U);
  ASSERT_EQ(static_cast<uint8_t>(buf->argv[5]), 5U);
  ASSERT_EQ(static_cast<uint8_t>(buf->argv[6]), 6U);
  ASSERT_EQ(static_cast<uint8_t>(buf->argv[7]), 0xf9U);
  ASSERT_EQ(static_cast<uint8_t>(buf->argv[8]), 0xf8U);
  ASSERT_EQ(static_cast<uint8_t>(buf->argv[9]), 9U);
  buf->argv[0] = 0xf6;
  ...
  ASSERT_EQ(static_cast<uint8_t>(buf->argv[6]), 6U);
  ASSERT_EQ(static_cast<uint8_t>(buf->argv[7]), 0xf9U);
  ASSERT_EQ(static_cast<uint8_t>(buf->stack_argv[0]), 0xf8U);
  ASSERT_EQ(static_cast<uint8_t>(buf->stack_argv[1]), 9U);
  buf->argv[0] = 0xf6;
#else
...
void Run10Int(GuestAddr pc, GuestArgumentBuffer* buf) {
  ...
  ASSERT_EQ(0u, buf->argv[0]);
  ASSERT_EQ(1u, buf->argv[1]);
  ASSERT_EQ(2u, buf->argv[2]);
  ASSERT_EQ(3u, buf->argv[3]);
  ASSERT_EQ(4u, buf->argv[4]);
  ASSERT_EQ(5u, buf->argv[5]);
  ASSERT_EQ(6u, buf->argv[6]);
  ASSERT_EQ(7u, buf->argv[7]);
  ASSERT_EQ(8u, buf->argv[8]);
  ASSERT_EQ(9u, buf->argv[9]);
  buf->argv[0] = 45;
#elif defined(NATIVE_BRIDGE_GUEST_ARCH_ARM64)
